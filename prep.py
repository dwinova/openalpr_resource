...
if command == 'neg':
    ...
    for neg_file in os.listdir(INPUT_NEGATIVE_DIR):
        if ' ' in neg_file:
            ...
            os.rename(INPUT_NEGATIVE_DIR + neg_file, INPUT_POSITIVE_DIR
                      + newfilename)
    ...
    for neg_file in os.listdir(INPUT_NEGATIVE_DIR):
        ...
        shutil.copy2(INPUT_NEGATIVE_DIR + neg_file, OUTPUT_NEGATIVE_DIR
                     + neg_file)
    ...
    for files in os.listdir(INPUT_POSITIVE_DIR):
        ...
        if ' ' in files:
            ...
            os.rename(INPUT_POSITIVE_DIR + files, INPUT_POSITIVE_DIR
                      + newfilename)

        # Copy from the raw directory to the positive directory

        shutil.copy2(INPUT_POSITIVE_DIR + newfilename,
                     OUTPUT_POSITIVE_DIR + newfilename)
    ...
    execStr = '%s/opencv_createsamples %s %s %s -num %d' % (OPENCV_DIR,
            vector_arg, width_height_arg, info_arg, total_pics)
    print execStr
    ...
    print 'SHOW'
    execStr = '%s/opencv_createsamples -vec %s -w %d -h %d' \
        % (OPENCV_DIR, VEC_FILE, WIDTH, HEIGHT)
    print execStr
    os.system(execStr)
elif command == 'train':
    ...
    execStr = \
        '%s/opencv_traincascade %s %s %s %s -numPos %d -numNeg %d -maxFalseAlarmRate 0.45 -featureType LBP -numStages 13' \
        % (
        OPENCV_DIR,
        data_arg,
        vector_arg,
